...
def retrieve(index, column):
    if replica.iloc[index][column] == np.NaN:
        if index == 0:
            return df.iloc[-1][column]
        else:
            return retrieve(index-1, column)
    else:
        return replica.iloc[index][column]
...
#for date in dates_for_update:
    ...
    #temperature_url = 'https://api.data.gov.sg/v1/environment/air-temperature?date=' + f'{yyyy}-{mm}-{dd}'
    #temp_json = requests.post(temperature_url).json()
    #d = json.dumps(temp_json, indent=4)
